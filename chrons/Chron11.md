...
**Status:** shipped — **Parachron 1.0.1 is released with all three assets**, built end to end by `release.yml`. Two criteria remain open and neither is writing: criterion 5 and criterion 6's dialog half both need a person at a Windows machine. See **How the criteria were verified**.
...
The remedy is `-C target-feature=+crt-static`, and it is **not** a one-line change: `mupdf-sys` compiles MuPDF's C and C++ through `cc`, which defaults to the dynamic runtime, and mixing `/MT` Rust with `/MD` C is its own class of link and heap bug. Doing it properly means forcing the C build to `/MT` in the same breath and re-running this spike to prove it.

**Settled for 1.0 by writing it down rather than by building differently.** The README's Windows section now names the redistributable, quotes the exact error Windows produces, and links Microsoft's installer. That was chosen over `+crt-static` on the reasoning this file has used throughout: an untested build change on the one target with no local machine is exactly the thing this milestone keeps refusing to ship. And the failure it describes is the good kind — Windows names the missing DLL on screen, so a user who hits it knows what to do rather than watching an app fail to open.

What that costs is honesty about a sentence: "there is nothing to install" is gone from the page, because it was not true. Static linkage is still the better answer and stays open past 1.0.
...
Of thirteen criteria: **six are closed** — 1 (a tag produced a three-asset release), 8 (green in CI on both Linux and Windows), 9 (6.6GB restored, 4m13s against ~18 cold), 11, 12 and 13. Three are verified in their build or layout half and open in their install half (2, 3, 4) — each needs root or a machine this project does not have. One is met for two packages of three and awaits the third being installed (7). **Two are not verified at all: 5 and 6's dialog half**, and both need the same thing — a person at a Windows machine with the `.exe` in front of them. Criterion 10 is struck with the AUR, which removes a row rather than answering one.

That is a milestone that shipped. It is not one that closed: the two open criteria are the two this file said from its first draft could never be closed from here, and they are still exactly that.
...
**The status is `shipped` rather than `done`, and the difference is one row of the acceptance table.** Of the three things that stood between here and closing, two are gone:
...
2. **Somebody runs the `.exe` on a real Windows machine** — for criterion 5 entirely, and for criterion 6's dialog half. **This is the whole of what is left.** The release page now carries the `.exe`, so it costs a download rather than a build, and it is a single sitting: launch it, look at the taskbar icon, open `Add Document`, pick a file.
3. ~~**The Visual C++ runtime question is answered**~~ — answered, though not by building differently. The README's Windows section names the redistributable, quotes the error Windows prints and links the installer. `+crt-static` with a matching `/MT` C build stays open past 1.0 and is recorded in CORE §7 as such: the better answer, not shipped unverified on the one target with no local machine.

**CORE §9's roadmap row now says the milestone shipped, because the Releases page is no longer empty.** What it does not say is `done`, and that is not modesty — criteria 5 and 6 are unobserved, and this project has spent eleven milestones not writing the convenient thing.

**One task for whoever closes it.** `publish` cannot attach an asset to a release that already exists (`gh release create` refuses it). If the two Windows criteria come back clean and nothing else needs cutting, that is a one-line change from `create` to a `create`-or-`upload` fallback — and it is worth doing before the next release rather than during one.
